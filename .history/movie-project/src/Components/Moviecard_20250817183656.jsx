...
const inWatchList=watchlist.some((m)=>m.id==movie.id)
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-black bg-opacity-80 text-white p-3  overflow-y-auto">
    <h3 className="font-bold mb-1 hidden group-hover:block">{movie.title}</h3>
    <p className="text-sm  hidden group-hover:block ">{movie.overview}</p>
  </div>
